# import the dataset
demographic <- read.csv("demographic_data.txt")
...
demographic[c(4, 100),]
demographic[1,]
demographic$MyCalc <- demographic$Birth.rate * demographic$Internet.users
demographic$xyz <- 1:5
demographic$MyCalc <- NULL
...
demographic[demographic$Income.Group == "High income",]
demographic[demographic$Country.Name == "Malta",]

# qplot
library(ggplot2)
qplot(data = demographic, x = Internet.users)
qplot(data = demographic, x = Income.Group, y = Birth.rate)
qplot(data = demographic, x = Income.Group, y = Birth.rate, size = I(3))
qplot(data = demographic, x = Income.Group, y = Birth.rate, size = I(3),
      colour=I("blue"))
qplot(data = demographic, x = Income.Group, y = Birth.rate, geom = "boxplot")
...
mydf <- data.frame(Coountry = Countries_2012_Dataset,
                   Code = Codes_2012_Dataset,
                   Region = Regions_2012_Dataset)

# merging dataframes
merged <- merge(x = demographic, y = mydf,
                by.x = "Country.Code", by.y = "Code")
merged$Coountry <- NULL
str(merged)
